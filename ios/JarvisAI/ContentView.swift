...
struct ContentView: View {
    ...
    @State private var backendAuthToken: String = ""
    ...
    var body: some View {
        ...
        .onChange(of: backendURL) { saveBackendURL($0) }
        .onChange(of: backendAuthToken) { saveBackendAuthToken($0) }
    ...
    private var apiSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            // Optional: route through a shared cloud backend instead of
            // calling Gemini directly from this device. Leave blank to
            // keep using the Gemini key above directly.
            VStack(alignment: .leading, spacing: 8) {
                Text("Cloud Backend (Optional)")
                ...
                VStack(spacing: 12) {
                    ...
                    APIInputField(
                        ...
                        placeholder: "Paste AUTH_TOKEN from your host",
                        text: $backendAuthToken,
                        icon: "lock"
                    )
                }
                .padding(14)
    ...
    private func saveBackendURL(_ url: String) {
        sharedDefaults?.set(url.trimmingCharacters(in: .whitespacesAndNewlines), forKey: AppConfig.Keys.backendURL)
    }

    private func saveBackendAuthToken(_ token: String) {
        sharedDefaults?.set(token.trimmingCharacters(in: .whitespacesAndNewlines), forKey: AppConfig.Keys.backendAuthToken)
    }
    ...
    private func loadSettings() {
        ...
        if let url = sharedDefaults?.string(forKey: AppConfig.Keys.backendURL) {
            self.backendURL = url
        }
        if let token = sharedDefaults?.string(forKey: AppConfig.Keys.backendAuthToken) {
            self.backendAuthToken = token
        }
